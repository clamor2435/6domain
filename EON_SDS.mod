...
param c{L} integer;           /*INDEX NOT USE*//* USING FOR LINK COST...? */
/*3-tuple Request*/
...
s.t. FIRST_INDEX_PRIMARY2{n in RN,f in {0..(F - r[n])},fp in {f..(f+r[n]-1)},i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}:
	xp[n,i,j,k,m,f] <= yp[n,i,j,k,m,fp];


s.t. FIRST_INDEX_BACKUP2{n in RN,f in {0..(F - r[n])},fp in {f..(f+r[n]-1)},i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}:
	xb[n,i,j,k,m,f] <= yb[n,i,j,k,m,fp];
...
s.t. INDEX_DISJOINT{i in DN,j in DN,k in NN,m in NN,f in FN:(i,j,k,m) in L}:
...
s.t. FIRST_INDEX_BACKUP4{n in RN,i in DN,j in DN, k in NN, m in NN:(i,j,k,m) in L}:
  sum{f in FN}(xb[n,i,j,k,m,f]) = (1 / r[n])*sum{f in FN}(yb[n,i,j,k,m,f]);
...
s.t. LIMIT_SUM_INDEX{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}:
	sum{f in FN}(sum{n in RN}(yp[n,i,j,k,m,f] + yb[n,i,j,k,m,f])) <= F;

s.t. LINK_DISJOINTNESS{n in RN,i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}:
	sum{f in FN}(xp[n,i,j,k,m,f]+xb[n,i,j,k,m,f])<=1;

s.t. SAME_DOMAIN_SEQUENCE{n in RN,i in DN, j in DN}:
	sum{k in NN, m in NN:(i,j,k,m) in L && i != j}sum{f in FN}(xp[n,i,j,k,m,f] - xb[n,i,j,k,m,f]) = 0;
